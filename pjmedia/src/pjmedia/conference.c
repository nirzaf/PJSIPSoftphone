			      ...
			      pjmedia_frame_type *frm_type)
{
    ...
    pj_int32_t tx_level;
	pj_int32_t tx_normalize_offset;
	pj_int64_t tx_normalize_sum;
    unsigned dst_count;
	...
	*frm_type = PJMEDIA_FRAME_TYPE_NONE;
	...
	for (j=0; j<conf->samples_per_frame; ++j) {
		tx_normalize_sum += (pj_int16_t) (cport->mix_buf[j]);
	}
	tx_normalize_offset = tx_normalize_sum / conf->samples_per_frame;
	...
	for (j=0; j<conf->samples_per_frame; ++j) {
		...
		if (buf[j]<tx_normalize_offset) {
			tx_level += tx_normalize_offset-buf[j];
		} else {
			tx_level += buf[j]-tx_normalize_offset;
		}
	}
    } else {
	for (j=0; j<conf->samples_per_frame; ++j) {
			     ...
			     pjmedia_frame *frame)
{
	...
	struct conf_port *conf_port = conf->ports[i];
	pj_int32_t level = 0;
	pj_int32_t rx_normalize_offset;
	pj_int64_t rx_normalize_sum;
	...
	rx_normalize_sum=0;
	for (j=0; j<conf->samples_per_frame; ++j) {
		rx_normalize_sum += p_in[j];
	}
	rx_normalize_offset = rx_normalize_sum / conf->samples_per_frame;
	...
	if (conf_port->rx_adj_level != NORMAL_LEVEL) {
		...
		if (p_in[j]<rx_normalize_offset) {
			level += rx_normalize_offset-p_in[j];
		} else {
			level += p_in[j]-rx_normalize_offset;
		}
	    }
	} else {
	    for (j=0; j<conf->samples_per_frame; ++j) {
