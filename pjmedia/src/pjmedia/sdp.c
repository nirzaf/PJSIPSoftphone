						 ...
						 pjmedia_sdp_media *m)
{
	...
	m->bandw_count = 0;
	m->desc.fmt_count = 1;
	pjmedia_sdp_attr *attr_rtpmap;
	pjmedia_sdp_attr *attr_fmtp;
	attr_rtpmap = pjmedia_sdp_media_find_attr2(m, "rtpmap",
		&m->desc.fmt[0]);
	attr_fmtp = pjmedia_sdp_media_find_attr2(m, "fmtp",
		&m->desc.fmt[0]);
	m->attr_count = 0;
	if (attr_rtpmap) {
		pjmedia_sdp_media_add_attr(m, attr_rtpmap);
	}
	if (attr_fmtp) {
		pjmedia_sdp_media_add_attr(m, attr_fmtp);
	}
    return PJ_SUCCESS;
}
