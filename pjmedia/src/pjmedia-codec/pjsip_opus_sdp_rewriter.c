...
#include <pjmedia-codec/pjsip_opus_sdp_rewriter.h>

/*
 * Only build this file if PJMEDIA_HAS_OUR_OPUS_CODEC != 0
 */
#if defined(PJMEDIA_HAS_OUR_OPUS_CODEC) && PJMEDIA_HAS_OUR_OPUS_CODEC != 0
...
static const pj_str_t STR_OPUS_RFC_FMT_ALT = { "opus/48000", 10 };
    ...
    if(rdata && rdata->msg_info.msg){
        ...
        if(opus_sdp_body_is_sdp(body)){
            ...
            if(found_opus != NULL){
				...
				found_opus = pj_stristr(&body_str, &STR_OPUS_RFC_FMT_ALT);
				if(found_opus != NULL){
					pj_str_t new_value;
					new_value.ptr = (char*) pj_pool_alloc(rdata->tp_info.pool, STR_OPUS_RFC_FMT_ALT.slen+1);
					new_value.slen = pj_ansi_snprintf(new_value.ptr, STR_OPUS_RFC_FMT_ALT.slen+1, "opus/%d", pjopus_internal_clockrate);
					while(new_value.slen < STR_OPUS_RFC_FMT_ALT.slen+1){
						new_value.ptr[new_value.slen ++] = ' ';
					}
					pj_memcpy(found_opus, new_value.ptr, new_value.slen - 1);
				}
