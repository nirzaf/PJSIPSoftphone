...
#   if PJMEDIA_HAS_INTEL_IPP_CODEC_GSMEFR
	{ 1, "GSM-EFR",	    PJMEDIA_RTP_PT_GSMEFR,       &USC_GSMAMR_Fxns,  8000, 1, 160,
			12200, 12200, 1, 0, 1,
			&predecode_amr, &parse_amr, &pack_amr,
			NULL
	},
...
#if PJMEDIA_HAS_INTEL_IPP_CODEC_AMR || PJMEDIA_HAS_INTEL_IPP_CODEC_GSMEFR || PJMEDIA_HAS_INTEL_IPP_CODEC_AMRWB
				   ...
				   pjmedia_codec_param *attr )
{
...
#if PJMEDIA_HAS_INTEL_IPP_CODEC_AMR
    /* Init AMR settings */
    if (ippc->pt == PJMEDIA_RTP_PT_AMR || ippc->pt == PJMEDIA_RTP_PT_GSMEFR || ippc->pt == PJMEDIA_RTP_PT_AMRWB) {
	amr_settings_t *s;
	...
	s->enc_setting.amr_nb = (pj_uint8_t)(ippc->pt == PJMEDIA_RTP_PT_AMR || ippc->pt == PJMEDIA_RTP_PT_GSMEFR);
	s->enc_setting.octet_aligned = octet_align;
	s->enc_setting.reorder = PJ_TRUE;
	if (ippc->pt == PJMEDIA_RTP_PT_GSMEFR) {
		s->enc_setting.cmr = 12;
	} else {
		s->enc_setting.cmr = 15;
	}

	s->dec_setting.amr_nb = (pj_uint8_t)(ippc->pt == PJMEDIA_RTP_PT_AMR || ippc->pt == PJMEDIA_RTP_PT_GSMEFR);
				     ...
				     struct pjmedia_frame *output)
{
	...
	if (pt == PJMEDIA_RTP_PT_AMR || pt == PJMEDIA_RTP_PT_GSMEFR || pt == PJMEDIA_RTP_PT_AMRWB) {
	    out.pBuffer += 2;
	}
#endif
