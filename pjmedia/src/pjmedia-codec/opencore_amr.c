...
#if defined(PJMEDIA_HAS_OPENCORE_GSMEFR_CODEC) && \
    (PJMEDIA_HAS_OPENCORE_GSMEFR_CODEC != 0)
#define USE_GSMEFR
#endif


#if defined(USE_AMRWB) || defined(USE_AMRWB) || defined(USE_GSMEFR)

#if defined USE_AMRNB || defined USE_GSMEFR
...
enum
{
	...
	IDX_GSM_EFR	/* Index for narrowband gsm-efr.      */
};

static pjmedia_codec_amr_config def_config[3] =
{{ /* AMR-NB */
...
static const pj_uint16_t* amr_bitrates[3] =
    {pjmedia_codec_amrnb_bitrates, pjmedia_codec_amrwb_bitrates, pjmedia_codec_amrnb_bitrates};

static const unsigned amr_bitrates_size[3] =
{
...
#endif
#ifdef USE_GSMEFR
	amr_codec_factory.init[IDX_GSM_EFR] = ((options & PJMEDIA_AMR_NO_GSMEFR) == 0);
#else
	amr_codec_factory.init[IDX_GSM_EFR] = PJ_FALSE;
#endif
...
#endif
#ifdef USE_GSMEFR
	if ((options & PJMEDIA_AMR_NO_GSMEFR) == 0) {
		pj_cstr(&codec_name, "GSM-EFR");
		status = pjmedia_sdp_neg_register_fmt_match_cb(
			&codec_name,
			&pjmedia_codec_amr_match_sdp);
		if (status != PJ_SUCCESS)
...
PJ_DEF(pj_status_t) pjmedia_codec_opencore_amrnb_deinit(void)
{
    if (amr_codec_factory.init[IDX_AMR_NB] &&
		...
		amr_codec_factory.init[IDX_GSM_EFR])
    {
...
PJ_DEF(pj_status_t) pjmedia_codec_opencore_gmsefr_set_config(
	const pjmedia_codec_gmsefr_config *config)
{
	return amr_set_config(IDX_GSM_EFR, (const pjmedia_codec_amr_config *)config);
}
				   ...
				   const pjmedia_codec_info *info )
{
	...
	const pj_str_t gmsefr_tag = { "GSM-EFR", 7 };
    ...
    /* Check payload type. */
    if (info->pt != PJMEDIA_RTP_PT_AMR && info->pt != PJMEDIA_RTP_PT_AMRWB && info->pt != PJMEDIA_RTP_PT_GSMEFR)
	return PJMEDIA_CODEC_EUNSUP;
    
    /* Check encoding name. */
    if (pj_stricmp(&info->encoding_name, &amr_tag) != 0 &&
        pj_stricmp(&info->encoding_name, &amrwb_tag) != 0 &&
		pj_stricmp(&info->encoding_name, &gmsefr_tag) != 0)
    {
	return PJMEDIA_CODEC_EUNSUP;
    }
    ...
    if ((info->clock_rate == 8000 && amr_codec_factory.init[IDX_AMR_NB]) ||
		...
		(info->clock_rate == 8000 && amr_codec_factory.init[IDX_GSM_EFR]))
    {
        return PJ_SUCCESS;
    }
				     ...
				     pjmedia_codec_param *attr )
{
	...
	if (id->pt == PJMEDIA_RTP_PT_GSMEFR) {
		idx = IDX_GSM_EFR;
				    ...
				    pjmedia_codec_info codecs[])
{
    ...
    *count = 0;
	...
	if (amr_codec_factory.init[IDX_GSM_EFR]) {
		pj_bzero(&codecs[*count], sizeof(pjmedia_codec_info));
		codecs[*count].encoding_name = pj_str("GSM-EFR");
		codecs[*count].pt = PJMEDIA_RTP_PT_GSMEFR;
		codecs[*count].type = PJMEDIA_TYPE_AUDIO;
		codecs[*count].clock_rate = 8000;
		codecs[*count].channel_cnt = 1;
		(*count)++;
				   ...
				   pjmedia_codec_param *attr )
{
	...
	if (attr->info.pt == PJMEDIA_RTP_PT_GSMEFR) {
		setting->cmr = 12;
	} else {
		setting->cmr = 15;
	}
