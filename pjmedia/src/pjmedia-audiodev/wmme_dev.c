					 ...
					 pjmedia_aud_stream **p_aud_strm)
{
	...
	stream_set_cap(&strm->base, PJMEDIA_AUD_DEV_CAP_INPUT_VOLUME_SETTING,
	       &param->input_vol);
    }
				  ...
				  const void *pval)
{
	...
	result = waveOutSetVolume(strm->play_strm.hWave.Out, waveVol);
	status = (result==MMSYSERR_NOERROR)? PJ_SUCCESS : 
				PJMEDIA_AUDIODEV_ERRNO_FROM_WMME_OUT(result);
	if (status == PJ_SUCCESS) {
		...
		strm->rec_strm.hWave.In)
	{
		...
		result = mixerOpen(&hMixer, (UINT)strm->rec_strm.hWave.In, 0, 0, MIXER_OBJECTF_HWAVEIN);
		...
		if (result==MMSYSERR_NOERROR) {
			//-- get the volume control of line
			...
			if (mixerGetLineControls((HMIXEROBJ) hMixer, 
				&mlc, MIXER_GETLINECONTROLSF_ONEBYTYPE)==MMSYSERR_NOERROR) {
					//-- set volume
					DWORD cChannels = ml.cChannels;
					if (MIXERCONTROL_CONTROLF_UNIFORM & mc.fdwControl)
						cChannels = 1;

					memset(&mcd, 0, sizeof(mcd));
					memset(&mcdu, 0, sizeof(mcdu));
					mcdu.dwValue = 655.35*vol; // the volume is a number between 0 and 65535
					...
					if (mixerSetControlDetails((HMIXEROBJ) hMixer, 
						&mcd, MIXER_SETCONTROLDETAILSF_VALUE)==MMSYSERR_NOERROR) {
							status = PJ_SUCCESS;
					}
					//--
			}
