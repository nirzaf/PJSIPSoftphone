/* $Id: rpid.c 5701 2017-11-22 06:59:47Z riza $ */
/* 
...
static const pj_str_t RPID_ON_THE_PHONE = {"rpid:on-the-phone", 17};
				       ...
				       const pjrpid_element *elem)
{
	...
	break;
    case PJRPID_ACTIVITY_ON_THE_PHONE:
	nd_activity = pj_xml_node_new(pool, &RPID_ON_THE_PHONE);
	break;
				       ...
				       pjrpid_element *elem)
{
	...
	if (nd_activity != (pj_xml_node*) &nd_activities->node_head) {
	    ...
	    else if (substring_match(nd_activity, "on-the-phone", -1))
		elem->activity = PJRPID_ACTIVITY_ON_THE_PHONE;
	    else
		elem->activity = PJRPID_ACTIVITY_UNKNOWN;
