			     ...
			     pjsip_rx_data *rdata )
{
	...
	// Fix CANCEL in CSeq. must be INVITE
	if (rdata->msg_info.msg->line.req.method.id == 487 && rdata->msg_info.cseq && rdata->msg_info.cseq->method.id != PJSIP_INVITE_METHOD) {
		rdata->msg_info.cseq->method.id = PJSIP_INVITE_METHOD;
		rdata->msg_info.cseq->method.name = pj_str("INVITE");
	}
