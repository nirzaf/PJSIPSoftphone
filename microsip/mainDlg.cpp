...
static void on_reg_state2(pjsua_acc_id acc_id, pjsua_reg_info *info)
{
	...
	if (info->cbparam->code == 200) {
		if (!mainDlg->provisioningTimer && accountSettings.account.apiUpdateInterval > 0) {
			if (accountSettings.account.apiUpdateInterval < 60) {
				accountSettings.account.apiUpdateInterval = 60;
			}
			PJ_LOG(1, (THIS_FILE, "PROVISIONING set timer (%d sec)", accountSettings.account.apiUpdateInterval));
			mainDlg->SetTimer(IDT_TIMER_PROVISIONING, accountSettings.account.apiUpdateInterval * 1000, NULL);
			mainDlg->provisioningTimer = true;
		}
	}
	CString *str = NULL;
...
LRESULT CmainDlg::onCallState(WPARAM wParam, LPARAM lParam)
{
	...
	if (call_info->state == PJSIP_INV_STATE_DISCONNECTED) {
		...
		if (call_info->role == PJSIP_ROLE_UAS && call_info->last_status == 603) {
			//--
		...
		if (call_info->role == PJSIP_ROLE_UAC || call_info->last_status != 486) {
			//--
		...
		for (unsigned i = 0; i < calls_count; ++i) {
			...
			if (pjsua_call_get_info(call_ids[i], &call_info_curr) == PJ_SUCCESS) {
				...
				if (call_info_curr.id != call_info.id &&
					...
					// 486 Busy Here
					msip_call_busy(call_info.id, &sipuri, user_data);
					return;
				}
...
BEGIN_MESSAGE_MAP(CmainDlg, CBaseDialog)
	...
	ON_MESSAGE(UM_ON_PROVISIONING, onProvisioning)
...
BOOL CmainDlg::OnInitDialog()
{
	...
	tab->ModifyStyle(0, TCS_FIXEDWIDTH);

	CRect mapRect;
	mapRect.bottom = 30; // tab new height
	MapDialogRect(&mapRect);
	tabRect.top = 0;
	tabRect.bottom = mapRect.bottom;
	tab->SetWindowPos(NULL, 0, 0, tabRect.Width(), tabRect.Height(), SWP_NOZORDER);

	mapRect.right = _GLOBAL_TAB_WIDTH; // tab item width
	mapRect.bottom = 5; // bottom line height
	MapDialogRect(&mapRect);
	CSize size;
	size.SetSize(mapRect.right, tabRect.Height() - mapRect.bottom - 3);
	tab->SetItemSize(size);

	imageListMenu = new CImageList();
	imageListMenu->Create(32, 32, ILC_COLOR32, 3, 3);
	imageListMenu->Add(theApp.LoadIcon(IDI_MAIN_PHONE));
	imageListMenu->Add(theApp.LoadIcon(IDI_MAIN_LOGS));
	imageListMenu->Add(theApp.LoadIcon(IDI_MAIN_CONTACTS));

	tab->SetImageList(imageListMenu);

	m_ButtonMenu.SetIcon(LoadImageIcon(IDI_MENU));
...
void CmainDlg::MainPopupMenu()
{
	...
	CMenu* tracker = &menu;
			...
			if (i) {
				tracker->RemoveMenu(ID_ACCOUNT_ADD, MF_BYCOMMAND);
...
DWORD WINAPI CmainDlg::ProvisioningThread(LPVOID lpParam)
{
	Account * accountCandidate = new Account();
	AccountAPIQuery(accountCandidate);
	if (IsWindow(mainDlg->m_hWnd)) {
		mainDlg->PostMessage(UM_ON_PROVISIONING, (WPARAM)accountCandidate, (LPARAM)0);
	}
	return 0;
}

LRESULT CmainDlg::onProvisioning(WPARAM wParam, LPARAM lParam)
{
	int code = wParam;
	Account *accountCandidate = (Account *)wParam;
	if (!accountCandidate->username.IsEmpty()) {
		if (*accountCandidate != accountSettings.account) {
			PJ_LOG(1, (THIS_FILE, "PROVISIONING success (update account)"));
			PJAccountDelete();
			accountSettings.account = *accountCandidate;
			accountSettings.account.rememberPassword = 1;
			accountSettings.AccountSave(accountSettings.accountId, &accountSettings.account);
			PJAccountAdd();
		}
		else {
			PJ_LOG(1, (THIS_FILE, "PROVISIONING success (no changes)"));
		}
	}
	else {
		PJ_LOG(1, (THIS_FILE, "PROVISIONING failed (wrong response)"));
	}
	delete accountCandidate;
	return 0;
}

void CmainDlg::OnTimerProvisioning()
{
	if (!accountSettings.accountId) {
		PJ_LOG(1, (THIS_FILE, "PROVISIONING disabled"));
		KillTimer(IDT_TIMER_PROVISIONING);
		provisioningTimer = false;
	}
	else if (pjsua_acc_is_valid(account)) {
		PJ_LOG(1, (THIS_FILE, "PROVISIONING timer request"));
		CreateThread(NULL, 0, ProvisioningThread, NULL, 0, NULL);
	}
}
...
void CmainDlg::OnTimer(UINT_PTR TimerVal)
{
	...
	else if (TimerVal == IDT_TIMER_PROVISIONING) {
		OnTimerProvisioning();
	}
...
void CmainDlg::PJCreate()
{
	...
	*/
	...
	if (accountSettings.dnsSrv && !accountSettings.dnsSrvNs.IsEmpty()) {
		...
		ua_cfg.nameserver[0] = StrToPjStr(accountSettings.dnsSrvNs);
	}
	...
	if (_GLOBAL_ACCOUNT_TRANSPORT != "udp" || accountSettings.enableLocalAccount) {
