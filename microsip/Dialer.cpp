...
BOOL Dialer::OnInitDialog()
{
	...
	m_hIcon = LoadImageIcon(IDI_LOGO);
	CRect rect;
	GetWindowRect(&rect);
	ScreenToClient(&rect);
	if (mainDlg->widthAdd) {
		rect.right-=mainDlg->widthAdd;
	}
	ICONINFO info;
	...
	logoRect.right = logoRect.left + bm.bmWidth;

	CRect mapRect;
	mapRect.bottom = 4;
	MapDialogRect(&mapRect);

	logoRect.bottom = rect.bottom - mapRect.bottom;
	logoRect.top = logoRect.bottom - bm.bmHeight;

	m_Logo.Create(_T("STATIC"), NULL, WS_CHILD, logoRect, this, IDC_LOGO);

	AutoMove(m_Logo.m_hWnd, 50, 100, 0, 0);
...
void Dialer::OnPaint()
{
	if (!IsIconic())
	{
		...
		ScreenToClient(rect);
		DrawIconEx(dc, rect.left, rect.top, m_hIcon, 0, 0, NULL, NULL, DI_MASK | DI_IMAGE | DI_NOMIRROR);
	}
	else
	{
		CBaseDialog::OnPaint();
...
void Dialer::OnRButtonUp(UINT nFlags, CPoint pt)
{
	...
	if (pt.y >= logoRect.top && pt.y <= logoRect.bottom
		&& pt.x >= logoRect.left && pt.x <= logoRect.right
		) {
		if (!nFlags) {
			mainDlg->MainPopupMenu();
		}
		else {
			mainDlg->OnMenuWebsite();
		}
	}
}
