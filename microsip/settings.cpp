...
void AccountSettings::Init()
{
	...
	//--
	ptr = str.GetBuffer(255);
	GetPrivateProfileString(section, _T("singleMode"), _T(_GLOBAL_SINGLE_MODE_DEFAULT), ptr, 256, iniFile);
	str.ReleaseBuffer();
...
bool AccountSettings::AccountLoad(int id, Account *account)
{
	...
	ptr = account->apiLogin.GetBuffer(255);
	GetPrivateProfileString(section, _T("apiLogin"), NULL, ptr, 256, iniFile);
	account->apiLogin.ReleaseBuffer();

	ptr = account->apiPassword.GetBuffer(1040);
	GetPrivateProfileString(section, _T("apiPassword"), NULL, ptr, 1041, iniFile);
	account->apiPassword.ReleaseBuffer();
	if (!account->apiPassword.IsEmpty()) {
		CByteArray arPassword;
		String2Bin(account->apiPassword, &arPassword);
		CCrypto crypto;
		if (crypto.DeriveKey((LPCTSTR)_GLOBAL_KEY)) {
			try {
				crypto.Decrypt(arPassword, account->apiPassword);
			}
	...
	account->rememberPassword = !account->apiLogin.GetLength() || account->apiPassword.GetLength() ? 1 : 0;
	...
	account->voicemailNumber = _T(_GLOBAL_ACCOUNT_VM_NUMBER);
	...
	return  sectionExists && (!account->apiLogin.IsEmpty() || !account->domain.IsEmpty());
		}
...
void AccountSettings::AccountSave(int id, Account *account)
{
	...
	if (!account->rememberPassword) {
		WritePrivateProfileString(section, _T("apiLogin"), _T(""), iniFile);
		WritePrivateProfileString(section, _T("apiPassword"), _T(""), iniFile);
	}
	else {
		WritePrivateProfileString(section, _T("apiLogin"), account->apiLogin, iniFile);
		CCrypto crypto;
		CByteArray arPassword;
		if (!account->apiPassword.IsEmpty() && crypto.DeriveKey((LPCTSTR)_GLOBAL_KEY)
			&& crypto.Encrypt(account->apiPassword, arPassword)
			) {
			str = Bin2String(&arPassword);
		}
		else {
			str = account->apiPassword;
		}
		WritePrivateProfileString(section, _T("apiPassword"), str, iniFile);
	}
