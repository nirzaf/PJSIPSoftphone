...
CString msip_url_mask(CString url)
{
	if (accountSettings.accountId) {
		url.Replace(_T("{apiLogin}"), CString(urlencode(Utf8EncodeUcs2(accountSettings.account.apiLogin))));
		url.Replace(_T("{apiPassword}"), CString(urlencode(Utf8EncodeUcs2(accountSettings.account.apiPassword))));
		url.Replace(_T("{md5_apiPassword}"), CString(msip_md5sum(&accountSettings.account.apiPassword)));
		...
		url.Replace(_T("{apiLogin}"), _T(""));
		url.Replace(_T("{apiPassword}"), _T(""));
		url.Replace(_T("{md5_apiPassword}"), _T(""));
